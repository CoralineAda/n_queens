class Solver
  ...
  def display_board
    puts self.board
    puts "Iteration #{self.iterations}: #{self.board.threatened_queens.count} threatened queens"
  end
...
class Board
  ...
  def move_queens(force=false)
    self.queens.each{|q| q.move}
    if (threatened = self.threatened_queens.first) && force
      threatened.move(force)
    end
  end
...
class Queen
  ...
  def move_to_random_square
    ...
    x_destination, y_destination = destinations[rand(destinations.count)]
    self.coords = [x_destination, y_destination]
    return self.board
  end
